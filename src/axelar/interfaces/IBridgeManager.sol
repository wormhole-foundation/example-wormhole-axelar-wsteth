...
interface IBridgeManager {
    ...
    ///         Finally, this function will call into the Bastion contracts to send a message with the incrementing msgId sequence number, msgType = 1, and the token transfer payload.
    function transfer(
        uint256 amount,
    ...
    /// @notice Called by a Bastion contract to deliver a verified attestation.
    ...
    /// @notice Returns the number of Bastions that must attest to a msgId for it to be considered valid and acted upon.
    function getThreshold() external view returns (uint8);

    /// @notice Returns the Bastion contracts that have been registered via governance.
    function getBastions() external view returns (address[] memory);
}
